...
export default (state: IGameKeyboard = initState, action: Action) =>
  produce(state, draft => {
    switch (action.type) {
      case types.KEY_DOWN:
        draft.down = action.payload;
        break;
      case types.KEY_LEFT:
        draft.left = action.payload;
        break;
      case types.KEY_RIGHT:
        draft.right = action.payload;
        break;
      case types.KEY_ROTATE:
        draft.rotate = action.payload;
        break;
      case types.KEY_DROP:
        draft.drop = action.payload;
        break;
      case types.KEY_RESET:
        draft.reset = action.payload;
        break;
